--[[ LuaOne | entry: bundler | modules: 7 | 2026-04-15T18:26:26Z ]]
...
__loaders__["source.discover"] = function(...)
   ...
   function Discover.run(entry, src, opts)
   		...
   		if follow_requires then
   			...
   			for _, req in ipairs(reqs) do
   				if req.kind == "static" then
   					if resolve_local(req.value) then
   	...
   	for _, name in ipairs(opts.extra_names or {}) do
   		if not resolve_local(name) then
   			print("WARN: extra_file '" .. name .. "' not found in " .. src)
   		else
...
__loaders__["source.cli"] = function(...)
   ...
   local HELP = table.concat({
   	...
   	"  lua bundler.lua --entry app --out bundle.lua --strip non_ann",
   ...
   function CLI.parse(args)
   	...
   	local config_path, detected_cfg = detect_config_path(flags.config_path)
   	local file_cfg = detected_cfg or {}
   	if config_path and not detected_cfg then
   		local ok, result = pcall(load_config, config_path)
   		if not ok then
   			error("[bundler] " .. tostring(result), 0)
   		end
   		file_cfg = result or {}
   	end
   	if config_path then
   	...
   	local cfg = merge(file_cfg, {
   		...
   		name = flags.name,
   		strip = flags.strip,
   		debug = flags.debug or nil,
   		verify = flags.verify or nil,
   	})
   	cfg.src = cfg.src or "./"
